  ...
  return (
    <header className='flex items-center justify-between px-5'>
      ...
      <nav className={isOpen ? "open" : ""}>
        <ul className='lg:flex lg:items-center lg:justify-center lg:gap-3'>
           <li>
           ...
           <li>
            <Link to="/dragon" className='text-white text-sm '>
            Dragons</Link>
           </li>
           <li>
      ...
      <div className='lg:hidden'>
          <button onClick={()=> setIsOpen(!isOpen)} className='text-white text-sm uppercase tracking-wider' >{isOpen ? "Close" : "open"}</button>
        </div>
    </header>
  )
}
